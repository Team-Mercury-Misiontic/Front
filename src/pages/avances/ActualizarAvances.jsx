...
const ActualizarAvances = () => {
  ...
  const {
    dataUsuario: queryDataUsuario,
    errorUsuario: queryErrorUsuario,
    loadingUsuario: queryLoadingUsuario,
  } = useQuery(GET_USUARIO, {
    variables: { _id },
  });
  const {
    dataProyecto: queryDataProyecto,
    errorProyecto: queryErrorProyecto,
    loadingProyecto: queryLoadingProyecto,
  } = useQuery(GET_PROYECTO, {
    variables: { _id },
  });
  const {
    dataAvances: queryDataAvances,
    errorAvances: queryErrorAvances,
    loadingAvances: queryLoadingAvances,
  ...
  const [
    actualizarAvance,
    {
      dataAvances: mutationDataAvances,
      loadingAvances: mutationLoadingAvances,
      errorAvances: mutationErrorAvances,
    },
  ...
  useEffect(() => {
    ...
    queryErrorAvances,
    mutationErrorAvances,
    queryErrorProyecto,
    queryErrorUsuario,
  ]);

  if (queryLoadingUsuario || queryLoadingProyecto || queryLoadingAvances)
    return (
    ...
    <div className="flew flex-col w-full h-full items-center justify-center p-10">
      ...
      >
        <span className="m-auto text-center rounded-md text-black text-lg">
          Proyecto: {queryDataProyecto.Proyecto.nombre}
        </span>
        <Input
          label="Fecha:"
          type="date"
        ...
        <Input
          ...
          name="creadoPor"
          defaultValue={
            queryDataUsuario.Usuario.nombre && queryDataUsuario.Usuario.apellido
          }
          required={true}
        />
        <ButtonLoading
